...
function animate() {
  ...
  //   let mesh = scene.getObjectByName(`box-0`);
  //   }else{    
  //   let mesh = scene.getObjectByName(`box-1`);
...
const addVideoNode = (participant, stream) => {
 ...
 if (participant.id === VoxeetSDK.session.participant.id) {
  ...
  mesh.material = videoMaterial;
    ...
    // videoScreenGeo.scale(0.5, 0.5, 0.5);
    // // participants++
    // let colorMaterial =  new THREE.MeshLambertMaterial({color: new THREE.Color().setHSL(0 / 3, 1, 0.5),})
    ...
    // // videoScreen.rotation.set(0,rotationOffsets[participantCount],tiltOffsets[participantCount] d);
...
const tokenServerURL =
  "https://dolby-io-tokenserver-lookingglass.netlify.app/api/token-generator";
let mediaDevices;
